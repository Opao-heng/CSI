...
def load_single_env_data(data_dir):
    ...
    读取单个env文件夹下的所有数据文件
    Args: 包含数据文件的目录路径
    Returns:(data_list, label_list) 数据和标签列表
    """
...
def load_combined_env_data(env0_dir, env1_dir):
    ...
    Returns:
        tuple: (source_data, source_labels)
            - source_data: 合并后的数据张量
            - source_labels: 合并后的标签张量
    """
...
if __name__ == "__main__":
    # 指定两个环境的数据目录
    env0_directory = "../data/env0"
    env1_directory = "../data/env1"
    
    # 加载并合并两个环境的数据
    source_data, source_labels = load_combined_env_data(env0_directory, env1_directory)
    
    if source_data is not None:
        # 现在source_data和source_labels可以作为源域数据使用
