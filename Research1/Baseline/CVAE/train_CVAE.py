...
from model_CVAE import build_cvae_model
from loss_CVAE import combined_vae_loss
from Research1.Process.dataloder_GAN import CustomDataset, select_samples_by_label
from plot_CVAE import (plot_training_metrics, save_evaluation_results)
from Research1.plot_GAN import plot_synthetic_sample_amplitude
from Research1.train_GAN import save_combined_target_data


"""
执行CVAE模型的完整训练流程
"""
def train_and_test(model_path='cvae_model.pth', epochs=100, lr=1e-3, num_samples=900, beta=1.0, lambda_freq=0.1, num_classes=30):
    # 步骤1: 初始化设备
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    
    # 步骤2: 构建模型
    model = build_cvae_model(num_classes=num_classes)
    model.to(device)
    
    # 步骤3: 打印模型参数统计
    total_params = sum(p.numel() for p in model.parameters())
    print(f"  CVAE模型参数量总量: {total_params:,}")
    ...
    # 步骤9: 绘制训练指标
    print(f"  正在绘制训练指标...")
    plot_training_metrics(train_loss_history, output_dir='CVAE')
    
    # 步骤10: 绘制生成样本的幅度图
    print(f"  正在绘制生成样本幅度图...")
    random_idx = np.random.randint(0, len(synthetic_data))
    plot_synthetic_sample_amplitude(synthetic_data, sample_idx=random_idx, output_dir='CVAE')
    ...
    save_evaluation_results(evaluation_results, train_loss_history, 'CVAE')
...
def generate_synthetic_samples(model, source_loader, num_samples=900, device='cuda'):
    ...
    with torch.no_grad():
        ...
        for x_s, source_labels in source_loader:
            ...
            # 生成样本 (带条件)
            generated = model.generate(x_s.size(0), source_labels, device)
            
            # 收集生成的样本和对应的标签
            synthetic_data.append(generated.cpu())
            synthetic_labels.append(source_labels.cpu()[:generated.size(0)])
...
if __name__ == "__main__":
    ...
    synthetic_data, synthetic_labels = train_and_test(
        model_path='CVAE/cvae_model.pth',  # 修改模型保存路径
        epochs=50,
        lr=5e-5,  # 进一步降低学习率以提高训练稳定性
        num_samples=900,
        beta=1.0,  # 恢复为1.0，现在损失函数已经优化
        lambda_freq=1.0,  # 增加频域损失权重
        num_classes=30
    )

    target_gan_data, target_gan_labels = save_combined_target_data(synthetic_data, synthetic_labels,
                                                                   target_loader, 'CVAE/')
