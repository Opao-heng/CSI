...
def save_gan_results(fid, inception_score, save_dir='model_F'):
    """
...
def train_and_test(model_path='model.pth', epochs=100, lr_g=1e-4, lr_d=1e-4, num_samples=900):
    ...
    # 步骤9: 保存评估结果
    save_gan_results(metrics.get('fid', -1), metrics.get('inception_score', -1), 'model_F')
...
if __name__ == "__main__":
    ...
    # 步骤6: 执行主训练流程
    print("步骤3: 开始GAN训练...")
    metrics = train_and_test(model_path='model_F/best_gan_model.pth', epochs=150, lr_g=1e-4, lr_d=1e-4, num_samples=800)
