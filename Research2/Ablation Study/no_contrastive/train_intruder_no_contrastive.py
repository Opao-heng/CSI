...
def train_and_evaluate_intruder_detection_with_ablation_model():
    ...
    # 消融实验标识：无对比损失版本
    print("=" * 60)
    print("消融实验：入侵者检测模型（基于无对比损失的身份识别模型）")
    print("训练配置：使用消融实验身份识别模型提取特征，重新训练入侵者检测器")
    print("=" * 60)
    ...
    # 加载模型权重
    model_path = os.path.join(os.path.dirname(__file__), "best_identify_model_no_contrastive.pth")  # 使用绝对路径
    if os.path.exists(model_path):
        ...
        # 尝试在当前目录查找
        alt_path = "best_identify_model_no_contrastive.pth"
    ...
    }, 'best_intruder_detector_no_contrastive.pth')
    print("入侵者检测模型已保存到 best_intruder_detector_no_contrastive.pth")
