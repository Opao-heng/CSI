...
from model_intruder_no_traditional import LearnableIntruderDetectorNoTraditional
...
def train_intruder_detector(model, identity_model, train_loader, val_loader, device, num_epochs=50):
    """
    训练入侵者检测器
    """
    model.train()
    identity_model.eval()  # 身份识别模型不训练
    
    # 定义损失函数和优化器
    criterion = torch.nn.BCEWithLogitsLoss()  # 使用BCEWithLogitsLoss处理二分类问题
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # 使用身份识别模型提取特征
            with torch.no_grad():  # 不计算身份识别模型的梯度
                identity_outputs = identity_model(data)
                features = identity_outputs['features']
                logits = identity_outputs['logits']
            ...
            optimizer.zero_grad()
            detector_outputs = model(features, logits, identity_labels)
            logits_output = detector_outputs['logits']  # 获取logits用于损失计算
            
            # 将标签转换为float类型以匹配BCEWithLogitsLoss的要求
            float_labels = labels.float()
            
            # 计算损失
            loss = criterion(logits_output, float_labels)
            loss.backward()
            optimizer.step()
        ...
        val_accuracy, val_f1, val_precision, val_recall = validate_intruder_detector(
            model, identity_model, val_loader, device)
...
def train_and_evaluate_intruder_detection_with_ablation_model():
    ...
    # 消融实验标识：无传统OpenMax版本
    print("=" * 60)
    print("消融实验：入侵者检测模型（无传统OpenMax组件）")
    print("训练配置：使用消融实验身份识别模型提取特征，重新训练仅包含可学习检测模块的入侵者检测器")
    print("=" * 60)
    ...
    # 初始化仅包含可学习检测模块的入侵者检测器
    print("初始化入侵者检测模型（无传统OpenMax组件）...")
    comprehensive_detector = LearnableIntruderDetectorNoTraditional(feature_dim=128).to(device)
    
    # 训练入侵者检测模型
    print("开始训练入侵者检测模型...")
    comprehensive_detector = train_intruder_detector(
        comprehensive_detector, identity_model, 
        data_loaders['intruder_train'], data_loaders['intruder_validation'], 
        device, num_epochs=50)
    
    # 保存训练好的入侵者检测模型
    torch.save({
        'model_state_dict': comprehensive_detector.state_dict(),
    }, 'best_intruder_detector_no_traditional.pth')
    print("入侵者检测模型已保存到 best_intruder_detector_no_traditional.pth")
